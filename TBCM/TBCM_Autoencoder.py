"""
Autoencoder-Based Domain Adaptation: BCM -> TBCM
...
All use shared encoder architecture:
...
N_RUNS = 3
DEVICE = torch.device('cpu')  # MPS can be flaky for small models
...
def train_mmd_ae(X_source, Y_source, X_target_train, Y_target_train,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(n_batches):
            ...
            # Reconstruction (both domains)
            recon_s = decoder(z_s)
            recon_t = decoder(z_t)
            loss_recon = mse(recon_s, xb_s) + mse(recon_t, xb_t)

            # Prediction (both domains have labels)
...
def train_daae(X_source, Y_source, X_target_train, Y_target_train,
    ...
    - Discriminator classifies domain from latent z
    - Encoder trained to fool discriminator (adversarial)
    ...
    for epoch in range(epochs):
        # Lambda schedule: ramp from 0 to lambda_max over warmup_epochs
        ...
        for batch_idx in range(n_batches):
            ...
            # Prediction loss (both domains)
            pred_s = predictor(z_s)
            pred_t = predictor(z_t)
            loss_pred = mse(pred_s, yb_s) + mse(pred_t, yb_t)

            # Adversarial loss: fool discriminator (reverse labels)
            disc_pred_s2 = discriminator(z_s)
            disc_pred_t2 = discriminator(z_t)
            # Encoder wants discriminator to be confused — reverse labels
            adv_labels_s = torch.full((len(z_s), 1), 0.1, device=DEVICE)  # source pretends target
            adv_labels_t = torch.full((len(z_t), 1), 0.9, device=DEVICE)  # target pretends source
        ...
        # Early stopping on prediction loss
...
# ==================== EXPERIMENT RUNNER ====================
def run_ae_experiment(df_bcm, df_tbcm, frac, scaler_X, scaler_Y,
                      random_state=42):
    """Run all 3 AE approaches at a given TBCM data fraction."""
    if frac < 1.0:
        df_target = df_tbcm.sample(frac=frac, random_state=random_state)
    else:
        df_target = df_tbcm.copy()

    X_target = df_target[['a_CT', 'a_TA', 'PS']].values
    Y_target = df_target[['F0', 'SPL']].values
    ...
    # Source data (already scaled during main setup)
    X_src_sc = scaler_X.transform(df_bcm[['a_CT', 'a_TA', 'PS']].values)
    Y_src_sc = scaler_Y.transform(df_bcm[['F0', 'SPL']].values)

    # Subsample source for efficiency (keep same size as full source or 10k max)
    ...
    results['latents'] = {
        'vanilla': latent_a, 'mmd': latent_b, 'daae': latent_c
    }
    results['loss_history'] = loss_hist

    # Get source latent for t-SNE comparison (using DAAE encoder)
    enc_c.eval()
    with torch.no_grad():
        src_sample_idx = rng.choice(len(X_src_sub), min(500, len(X_src_sub)), replace=False)
        tgt_sample_idx = rng.choice(len(X_tgt_test_sc), min(500, len(X_tgt_test_sc)), replace=False)
        z_src = enc_c(torch.FloatTensor(X_src_sub[src_sample_idx]).to(DEVICE)).cpu().numpy()
        z_tgt = enc_c(torch.FloatTensor(X_tgt_test_sc[tgt_sample_idx]).to(DEVICE)).cpu().numpy()
...
def main():
    print("=" * 70)
    print("AUTOENCODER DOMAIN ADAPTATION: BCM -> TBCM")
    print("=" * 70)
    ...
    print(f"  TBCM: {len(df_tbcm)} samples")
    print(f"  Device: {DEVICE}")

    # Fit scalers on source (BCM) data
    scaler_X = StandardScaler()
    scaler_Y = StandardScaler()
    scaler_X.fit(df_bcm[['a_CT', 'a_TA', 'PS']].values)
    scaler_Y.fit(df_bcm[['F0', 'SPL']].values)
    ...
    for frac in FRACTIONS:
        ...
        for run in range(N_RUNS):
            ...
            result = run_ae_experiment(df_bcm, df_tbcm, frac, scaler_X, scaler_Y, rs)

            if result is not None:
                frac_results.append(result)
                print(f"Vanilla={result['vanilla_f0_r2']:.3f}/{result['vanilla_spl_r2']:.3f}, "
                      f"MMD={result['mmd_f0_r2']:.3f}/{result['mmd_spl_r2']:.3f}, "
                      f"DAAE={result['daae_f0_r2']:.3f}/{result['daae_spl_r2']:.3f}")

                # Save last run's data for plotting
                if run == 0:
    ...
    for r in all_results:
        print(f"{r['frac']*100:>4.0f}% {r['n_samples']:>6} "
              f"{r['vanilla_f0_r2_mean']:>10.3f} {r['vanilla_spl_r2_mean']:>11.3f} "
              f"{r['mmd_f0_r2_mean']:>8.3f} {r['mmd_spl_r2_mean']:>9.3f} "
              f"{r['daae_f0_r2_mean']:>9.3f} {r['daae_spl_r2_mean']:>10.3f}")
    ...
    for r in all_results:
        methods = {
            'Vanilla': (r['vanilla_f0_r2_mean'] + r['vanilla_spl_r2_mean']) / 2,
    ...
    # ==================== SAVE RESULTS ====================
    results_path = os.path.join(script_dir, 'results', 'autoencoder_results.json')
    ...
    fig.suptitle('Autoencoder Domain Adaptation: BCM -> TBCM',
    ...
    plt.tight_layout()
    fig_path = os.path.join(figs_dir, 'tbcm_ae_results.png')
    ...
    if last_tsne is not None:
        ...
        fig2.suptitle('DAAE Latent Space: BCM vs TBCM (t-SNE)',
                      fontsize=14, fontweight='bold')
        ...
        if len(z_combined) > 0 and z_combined.shape[1] > 1:
            ...
            axes2[0].scatter(z_2d[n_src_pts:, 0], z_2d[n_src_pts:, 1],
                            alpha=0.4, s=10, c='red', label='TBCM (target)')
        ...
        # Density plot
        if len(z_combined) > 0:
            axes2[1].hist(z_src.flatten(), bins=50, alpha=0.5, density=True,
                         color='blue', label='BCM')
            axes2[1].hist(z_tgt.flatten(), bins=50, alpha=0.5, density=True,
                         color='red', label='TBCM')
        ...
        plt.tight_layout()
        fig2_path = os.path.join(figs_dir, 'tbcm_ae_latent.png')
    ...
    if last_loss_hist is not None:
        ...
        fig3.suptitle('DAAE Training Curves', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        fig3_path = os.path.join(figs_dir, 'tbcm_ae_training.png')
