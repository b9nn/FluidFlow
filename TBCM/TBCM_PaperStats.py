"""
TBCM (aligned target) master statistics run for the paper -- Table 2.

Same seven configurations and the same protocol as Beam_Membrane/BM_PaperStats.py,
so the two targets are directly comparable. TBCM carries only F0 and SPL.

Target : dataset_TBCM.csv (43,102 rows; PL dropped per the fixed input schema)
Source : data_binary.parquet (360,750-row BCM)

Writes results/tbcm_paper_stats.json incrementally.
"""
...
OUT_JSON = os.path.join(SCRIPT_DIR, "results", "tbcm_paper_stats.json")
...
def load_data():
    ...
    t = pd.read_csv(os.path.join(SCRIPT_DIR, "dataset_TBCM.csv"), index_col=0)
    t = t.rename(columns={"Ps": "PS"})
    t = t.dropna(subset=OUTPUTS)[FEATURES + OUTPUTS].reset_index(drop=True)
    print(f"  source {len(src)} rows | target {len(t)} rows")
    return src, t
...
def main():
    ...
    t_start = time.time()
    print("=" * 72)
    print("TBCM paper statistics: 7 configurations + source-alone")
    print("=" * 72)
    ...
    print("\nFitting BCM source models (once)...")
    source_models = {}
    for fam in ["PR", "RF", "NN"]:
        t0 = time.time()
    ...
    results = {
        "_meta": {
            "n_grid": N_GRID, "n_seeds": N_SEEDS, "test_pool": TEST_POOL,
            "outputs": OUTPUTS, "features": FEATURES,
            "target": "dataset_TBCM.csv", "target_rows": int(len(tgt)),
            "source": "data_binary.parquet", "source_rows": int(len(src)),
    ...
    for n in N_GRID:
        ...
        for s in range(N_SEEDS):
            rs = RANDOM_STATE + s
            tr = tgt.sample(n=n, random_state=rs)
            pool = tgt.drop(tr.index)
