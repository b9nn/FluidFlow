...
def main():
    ...
    rf_all = []
    ae_all = []
    # Per-replicate capture for cross-domain JSON dump (added 2026-05-13)
    per_replicate_rf = {}  # {N: list of run dicts with *_f0 / *_spl keys}
    ...
    for n in N_TARGETS:
        ...
        if rf_runs:
            per_replicate_rf[n] = list(rf_runs)
    ...
    # ==================== JSON DUMP (added 2026-05-13) ====================
    # Per-N small-N RF transfer dump for the cross-domain plot consumer
    # (Beam_Membrane/BM_CrossDomain.py). Schema matches alternates_results.json:
    #   {method: {'<N>': {'r2_f0': [list], 'r2_spl': [list], 'n_train': int}}}
    print("\nWriting small-N transfer JSON for cross-domain consumer...")
    ...
    small_n_payload = {
        '_meta': {
            ...
            'note': 'Per-N small-N RF transfer (source: BCM, target: TBCM). '
                    'Generated by TBCM_SmallData.py, mirroring '
                    'alternates_results.json N grid for direct comparison.',
        },
    }
    for method in ['source', 'target', 'residual', 'augmented', 'transrf']:
        block = {}
        for n in sorted(per_replicate_rf):
            runs = per_replicate_rf[n]
            block[str(n)] = {
                'r2_f0': [float(r[f'{method}_f0']) for r in runs],
                'r2_spl': [float(r[f'{method}_spl']) for r in runs],
                'n_train': int(runs[0]['n_train']),
            }
        small_n_payload[method] = block

    small_n_path = os.path.join(script_dir, 'results', 'rf_transfer_small_n.json')
    os.makedirs(os.path.dirname(small_n_path), exist_ok=True)
    with open(small_n_path, 'w') as _f:
        _json.dump(small_n_payload, _f, indent=2)
    print(f"  Wrote: {small_n_path}")
