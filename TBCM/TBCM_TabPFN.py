"""
BCM -> TBCM: TabPFN baseline (non-transfer)

Mirrors Beam_Membrane/BM_TabPFN.py — same backend selection (tabpfn-client
preferred, local tabpfn fallback), same harness, same N grid. Companion
to TBCM_GP.py.
...
def load_tbcm():
    ...
    if not os.path.exists(p):
        raise FileNotFoundError(f"TBCM dataset not found at {p}")
...
def main():
    print("=" * 70)
    print(f"TBCM ALTERNATES -- TabPFN (backend: {_TABPFN_BACKEND})")
    print("=" * 70)
    df = load_tbcm()
    print(f"  rows: {len(df)}")
